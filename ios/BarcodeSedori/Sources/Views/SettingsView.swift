        ...
        NavigationView {
            Form {
                ...
                Section("開発者向け") {
                    ...
                    Text("グラフの掃除(古いファイルの削除)にかかる時間を測るためのものです。上限5,000件+猶予200件ぶんを作るので、この直後に1件スキャンすると掃除が走ります。約230MBを書き込むため生成には時間がかかり、その間ストレージも消費します。履歴のダミーと両方を生成すると、商品タブの詳細画面でこのダミーグラフが表示され、再起動しても残ることを目視で確認できます。終わったら 設定→検索→「検索履歴を削除」で消してください。")
                        .font(.footnote)
                        .foregroundColor(.secondary)
                }
                #endif
    ...
    private var searchSection: some View {
        Section("検索") {
            ...
            .alert("検索履歴を削除しますか？", isPresented: $showHistoryDeleteConfirm) {
                ...
                Text("履歴データを全て削除します。この操作は取り消せません。実行しますか？")
            }

            // 上限の存在は利用者に見えないと「古い履歴が勝手に消えた」と受け取られるため、
            // 削除ボタンのすぐ下に常時表示しておく。件数はコード側の定数から組み立て、
            // 上限を変えたときに文言だけ古くなることを防ぐ。
            Text("\(ScanHistoryStore.maxItems)件以降の履歴は古い順に削除されます")
                .font(.footnote)
                .foregroundColor(.secondary)
        }
    }
