...
## 2026-09-11 履歴のランキングミニグラフ

- 商品画像（50pt幅）の下に、同じ幅・高さ24ptの緑色ランキング折れ線を追加。軸・価格系列・ラベルは表示しない。
- GraphArchiveの保存済みデータから表示時点の直近1年だけを抽出。保存がない・有効点が足りない場合は空白。API通信や無料枠消費は行わない。
- 表示された行だけ非同期でファイルをデコードし、画面外では描画用データを解放。欠損値で線を分断し、一定値は中央に描画。
- Simulator Debugビルド成功。実機の見た目・スクロール性能は未確認。ビルド番号の変更・アップロードは未実施。
