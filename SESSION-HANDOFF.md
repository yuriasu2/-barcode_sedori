...
- 商品画像（50pt幅）の下に、同じ幅・高さ31.2ptの緑色ランキング折れ線を追加。軸・価格系列・ラベルは表示しない。
- 外枠、横線4本、縦点線4本を追加。グラフ取得中は「読込中」、保存済みグラフがない場合は「未取得」と表示する。
- GraphArchiveの保存済みデータから表示時点の直近1年だけを抽出。API通信や無料枠消費は行わない。
- 表示された行だけ非同期でファイルをデコードし、画面外では描画用データを解放。欠損値で線を分断し、一定値は中央に描画。
- generic iOS Simulator Debugビルド、Releaseアーカイブ、App Store Connectアップロードが成功。1.0.1/build13は処理状態VALIDで、App Store Connectのバージョン1.0.1へ紐付け済み。実機の見た目・スクロール性能は未確認。
...
- `/health` 200、購入session APIも200でセッション発行成功。不正購入証明は400 `invalid_purchase`で拒否。DOは本番と分離。詳細は `docs/BILLING-OPERATIONS.md`。
- `~/Downloads/セラーレンズ課金キー.p8`（ファイル名NFD）を発見。本番WorkerのKey ID/Issuer IDとの組み合わせでApple Sandbox通知履歴APIへの認証に成功。
- 当初の秘密情報コピーは自動承認審査で拒否されたが、その後ユーザーが宛先を指定した鍵登録を明示承認。Apple課金キー3項目と検証専用の署名設定2項目を登録済み。署名鍵IDは`staging-v1`。Amazon秘密・本番署名鍵はコピーしていない。
...
- `server/src/billing/`、`BillingDO`、購入検証/更新/匿名セッション/通知V2を実装。全Proゲートは署名検証済みコンテキストのみ参照。旧`X-App-Plan`は信用しない。観測/自己申告へ戻す設定は設けていない。
...
- 次は [運用手順](docs/BILLING-OPERATIONS.md) のSecrets設定と実機確認。実機検証を終えるまで本番へデプロイしない。プライバシーポリシー追記案も同資料に記載（サイト未更新）。
- 以下の「企画書のみ」の段落は実装前の履歴。
