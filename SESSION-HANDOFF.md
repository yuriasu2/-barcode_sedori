...
- 告知を出す(本番KVなので`--remote`必須):
  ```bash
  cd server && npx wrangler kv key put --remote --binding ADS_CONFIG notice '{"id":"2026-08-13-keepa-outage","active":true,"title":"グラフの表示に不具合が発生しています","body":"現在、価格グラフが表示されない場合があります。","url":"https://sellira.jp/amalens/news/"}'
  ```
- 止める: `active` を `false` にして書き直す。**60秒のサーバーキャッシュがある**ため反映に最大1分かかる。
- `id`を変えない限り、一度閉じたユーザーには二度と出ない(端末のUserDefaultsで既読管理)。**内容を差し替えて再度出したいときは必ず`id`も変えること。**
