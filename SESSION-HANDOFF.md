...
- **サーバーの手動デプロイが必要**: `6612ef4`〜`4787098`(お試し期間のサーバー権威化、`SellerTrialDO`新設)がまだ本番へデプロイされていない。デプロイしないとAmazon連携の7日間お試しが誰にも付与されない(フェイルクローズ設計のため安全ではあるが、機能が使えない)。
  ```bash
...
### 3.【要対応・見積もり済み・未着手】プライバシーポリシーの公開

`https://sellira.jp/privacy/`は**まだ存在しない**。草案を `PRIVACY-POLICY-DRAFT.md` に用意済み(事業者名等の`●●`部分のみ穴埋めが必要)。課金画面から直接リンクしているため、**未公開のままだとApple審査に落ちる**。sellira-siteリポジトリでの対応が必要。
