...
| `settings_bottom` | 設定タブ下部 | 全員(Pro含む) | **50pt固定** |
...
- 設定タブは大タイトル「設定」(navigationTitle)を削除して広告分の余白を確保する
...
| 種類 | 扱い |
|---|---|
| 楽天アフィリエイト | 最優先。結果カードの `a/m/価` の隣に「楽」ボタンも追加(枠とは別の常設導線) |
...
1. 楽天アフィリエイト「楽」ボタン(枠システムと独立・最小で収益化開始)
2. サーバー `/api/ads` + KV + 計測エンドポイント + テスト
3. iOS AdsConfigStore + AdSlotView + BannerAdViewのadaptive/unitId対応
4. 下部固定枠3つ + 設定タブのタイトル削除 + ペイウォール文言修正
5. 運用: KVに広告在庫を投入、ASP案件の獲得
