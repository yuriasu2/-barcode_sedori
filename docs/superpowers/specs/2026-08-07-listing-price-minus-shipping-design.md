...
- 表示条件: `viewModel.showsSubtractedShippingRow`(**新規追加モード かつ 設定オン**)のときだけ表示する。オフのときは行ごと出さない(差し引きが起きていないのに配送料だけ出ていると意味が伝わらないため)。編集モードでも出さない: 編集時は保存済み価格をそのまま表示しており差し引きは起きていないため、出すと「最安値−配送料」で決まった価格だと誤認させる
- 表示値: `Self.currencyText(viewModel.shippingToSubtract)`(FBA利用時は自動的に`¥0`になる)
- ラベル: 「**差し引いた配送料**」。利益セクションには編集可能な「配送料」行(`shippingIncome`)が別にあり、利用者がそちらを編集すると2つの行が同じラベルで違う数字を示してしまうため、こちらは差し引いた実績値であることが分かる名前にする
...
`listingSection` の Pro 分岐内、`Toggle("FBAを利用", isOn: $settings.purchaseUseFbaDefault)` の直下に追加する。
...
2. 設定「利益計算用送料」の配送料を¥350にし、トグルON。最安値¥1000の商品で出品価格が¥650になり、直下に「配送料 ¥350」が出る
...
6. 仕入れリストの既存項目を開いて編集した場合、保存済みの価格が上書きされない。また「差し引いた配送料」行が出ない(編集時は差し引きが起きていないため)
