...
- 表示値: `Self.currencyText(viewModel.shippingToSubtract)`(設定「送料設定」の配送料デフォルト。FBA利用時は自動的に`¥0`になる)。編集済みアイテムの個別`shippingIncome`(利益セクションの編集可能な配送料)とは値がズレる場合があるが、これは意図した仕様(ユーザー確認済み)
- ラベル: 「配送料」
...
6. 仕入れリストの既存項目を開いて編集した場合、保存済みの価格が上書きされない。配送料行は編集モードでも表示される(設定の配送料デフォルトを表示。個別に編集した配送料とは異なる場合がある)
