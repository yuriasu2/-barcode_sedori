...
| フォームでFBAトグルを切替えたとき | **出品価格を再計算する** | 切替で配送料の扱いが変わるため。コンディション変更時に再計算する既存挙動と揃える |
...
**(c) `useFba` の `didSet`(新規。`.add` のときのみ)**
...
- 表示条件: `viewModel.subtractShippingFromLowest` が**オンのときだけ**表示する。オフのときは行ごと出さない(差し引きが起きていないのに配送料だけ出ていると意味が伝わらないため)
