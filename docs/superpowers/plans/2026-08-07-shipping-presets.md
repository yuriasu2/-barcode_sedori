...
- サーバー側は変更しない。デプロイ不要。
- Xcodeプロジェクトファイルは XcodeGen 生成でgit非追跡。`project.yml`が`sources: - path: Sources`とディレクトリ指定なので、新規ファイルを`.xcodeproj`へ**手動登録する作業は不要**。ただし`.xcodeproj`は静的な生成物でビルド前の自動再生成が無いため、**新規ファイルを追加したら`cd ios/BarcodeSedori && xcodegen generate`を実行する**こと(これを忘れると「Build input file cannot be found」または未定義シンボルでビルドが落ちる)。
